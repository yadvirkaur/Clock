...
function setAlarm() {
    if (isAlarmSet) {    //if button is clicked we will first check if alarm is set, if true that means btn is clicked to clear alarm
...
stopwatch.addEventListener('click',()=> {
  ...
  startBtn.addEventListener("click", () =>{
      if (startBtn.textContent === "START") {
        ...
        stoptimer=true;
        int= setInterval(stopwatchStart, 10);
     } 
      else {
  ...
  function stopwatchStart(){
     if(stoptimer){
      ...
      if(milli == 100){
        ...
        let ms = milli < 10 ? "0" + milli : milli;

        stopwatchDisplayTime.innerHTML = ` ${h}:${m}:${s}:${ms}`;
        // stopwatchDisplayTime.innerHTML = ` ${h}:${m}:${s}`;
        }
    } 
